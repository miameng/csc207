...
public class Heap<T extends HeapItem> {
	...
	private void sortUp(T item) {
		...
		for (; item.getHeapIndex() / 2 > 0; ) {
			...
			if (item.compareTo(father) < 0) {
				...
				items[father.getHeapIndex()] = father;
				item.setHeapIndex(item.getHeapIndex() / 2);
				items[item.getHeapIndex()] = item;
			} else break;
		}
	}
	...
	private void sortDown(T item) {
		...
		for ( ; item.getHeapIndex() * 2 <= currentItemCount; ) {
			int left = item.getHeapIndex() * 2;
			T son = left < currentItemCount ?
				items[left].compareTo(items[left + 1]) < 0 ?
					items[left] : items[left + 1] :
				items[left];
			if (son.compareTo(item) < 0) {
				item.setHeapIndex(son.getHeapIndex());
				items[item.getHeapIndex()] = item;
				son.setHeapIndex(son.getHeapIndex() / 2);
				items[son.getHeapIndex()] = son;
			} else break;
		}
	}
